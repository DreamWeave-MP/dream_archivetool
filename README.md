...
Archive creation and update preflight archive paths and format policy before adding deferred payload sources to the backend builders. TES3, TES4, and BA2 GNRL creation record filesystem paths and stream payloads while writing. `add` preserves unchanged TES3, TES4, and BA2 GNRL entries from the source archive through deferred archive-entry sources instead of decoding them into `dream-archivetool` memory. BA2 DX10 still has to parse DDS texture data through the backend texture builder, so it is not the same no-payload-buffering path.
